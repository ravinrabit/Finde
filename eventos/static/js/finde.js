(function () {
    ...
    function carregarLeaflet() {
        ...
        const prontoEstilo = new Promise((resolver) => {
            estilo.onload = resolver;
            estilo.onerror = resolver;
        });
        document.head.appendChild(estilo);

        const script = document.createElement("script");
        script.src = "https://unpkg.com/leaflet@1.9.4/dist/leaflet.js";
        const prontoScript = new Promise((resolver, rejeitar) => {
            script.onload = resolver;
            script.onerror = rejeitar;
        });
        document.head.appendChild(script);

        return Promise.all([prontoEstilo, prontoScript]);
    }

    function mesclarQuerystring(fonte) {
        const destino = new URL(fonte, window.location.origin);
    ...
    async function desenharMapa(alvo) {
        ...
        const resposta = await fetch(mesclarQuerystring(fonte), {
            headers: { "X-Requested-With": "XMLHttpRequest" },
        });
        if (!resposta.ok) throw new Error("mapa: resposta não OK");
        const dados = await resposta.json();
        const marcadores = [];
    ...
    /* =========================
       PREFERÊNCIAS (tema, fonte, contraste)
       ========================= */

    const CHAVE_PREFERENCIAS = "finde:preferencias";
    ...
    function salvarPreferencias(prefs) {
        try {
            ...
            // Aba privada ou storage bloqueado: a preferência só não persiste.
        }
    }

    function aplicarPreferencias(prefs) {
        const raiz = document.documentElement;
        if (prefs.tema && prefs.tema !== "sistema") raiz.dataset.tema = prefs.tema;
        else delete raiz.dataset.tema;
        raiz.dataset.fonte = prefs.fonte || "normal";
        if (prefs.contraste) raiz.dataset.contraste = "alto";
        else delete raiz.dataset.contraste;
    }

    function iniciarPreferencias() {
        const raiz = $("[data-preferencias]");
        const gatilho = $("[data-preferencias-gatilho]");
        const painel = $("[data-preferencias-painel]");
        if (!raiz || !gatilho || !painel) return;
        ...
        document.addEventListener("click", (evento) => {
            if (!painel.hidden && !raiz.contains(evento.target)) fechar();
        });
        document.addEventListener("keydown", (evento) => {
        ...
        const atualizarEstado = () => {
            $$("[data-tema-opcao]", painel).forEach((botao) => {
                botao.setAttribute("aria-pressed", String(botao.dataset.temaOpcao === (prefs.tema || "sistema")));
            });
            $$("[data-fonte-opcao]", painel).forEach((botao) => {
                botao.setAttribute("aria-pressed", String(botao.dataset.fonteOpcao === (prefs.fonte || "normal")));
            });
        };

        $$("[data-tema-opcao]", painel).forEach((botao) => {
            botao.addEventListener("click", () => {
                prefs.tema = botao.dataset.temaOpcao;
                aplicarPreferencias(prefs);
        ...
        $$("[data-fonte-opcao]", painel).forEach((botao) => {
            botao.addEventListener("click", () => {
                prefs.fonte = botao.dataset.fonteOpcao;
                aplicarPreferencias(prefs);
                salvarPreferencias(prefs);
                atualizarEstado();
            });
        });
        const contraste = $("[data-contraste-opcao]", painel);
        if (contraste) {
            contraste.checked = !!prefs.contraste;
            contraste.addEventListener("change", () => {
                prefs.contraste = contraste.checked;
                aplicarPreferencias(prefs);
                salvarPreferencias(prefs);
            });
        }
    ...
    function iniciarAssistente() {
        const raiz = $("[data-assistente]");
        const gatilho = $("[data-assistente-gatilho]");
        const painel = $("[data-assistente-painel]");
        const botaoFechar = $("[data-assistente-fechar]");
        const mensagens = $("[data-assistente-mensagens]");
        const formulario = $("[data-assistente-form]");
        if (!raiz || !gatilho || !painel || !formulario || !mensagens) return;
        ...
        const abrir = () => {
            painel.hidden = false;
            gatilho.setAttribute("aria-expanded", "true");
            const campo = $("input[name='pergunta']", formulario);
        ...
        function adicionarMensagem(texto, tipo) {
            ...
            paragrafo.className = "assistente-mensagem assistente-mensagem-" + tipo;
        ...
        formulario.addEventListener("submit", async (evento) => {
            ...
            try {
                ...
                const dados = await resposta.json();
                if (!resposta.ok) {
                    adicionarMensagem(dados.erro || "Não consegui responder agora.", "erro");
                } else {
                ...
                adicionarMensagem("Não consegui falar com o assistente agora.", "erro");
            } finally {
