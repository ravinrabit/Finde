...
from .services import moderacao
...
class EventoAdmin(admin.ModelAdmin):
    ...
    def rejeitar(self, request, queryset):
        eventos = list(queryset.select_related("criado_por", "produtor"))
        sem_motivo = sum(1 for evento in eventos if not evento.motivo_rejeicao)
        for evento in eventos:
            moderacao.rejeitar(evento, request=request)
        self.message_user(request, f"{len(eventos)} evento(s) rejeitado(s).")
        if sem_motivo:
            self.message_user(
                request,
...
@admin.register(SolicitacaoDestaque)
class SolicitacaoDestaqueAdmin(admin.ModelAdmin):
    list_display = ("evento", "plano", "criado_em", "atendida")
    list_filter = ("plano", "atendida")
    search_fields = ("evento__nome",)
    autocomplete_fields = ("evento",)
    readonly_fields = ("criado_em", "atendida_em")
