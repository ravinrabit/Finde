...
class CadastroDeEventoTests(TestCase):
    ...
    def test_evento_criado_ja_ganha_marcador_no_mapa(self):
        self.client.post(reverse("criar_evento"), self.dados())
        evento = Evento.objects.get(nome="Festival Novo de Teste")
...
class PaginasTests(TestCase):
    def test_paginas_publicas_respondem(self):
        paginas = [
            "home", "lista_eventos", "ajuda", "sobre", "termos", "privacidade",
            "acessibilidade", "eventos_fim_de_semana", "eventos_gratuitos",
        ]
        for nome in paginas:
            with self.subTest(pagina=nome):
                self.assertEqual(self.client.get(reverse(nome)).status_code, 200)

    def test_navbar_nao_mostra_fim_de_semana_nem_gratuitos(self):
        resposta = self.client.get(reverse("home"))
        self.assertNotContains(resposta, "Fim de semana")
        self.assertNotContains(resposta, "Gratuitos")
