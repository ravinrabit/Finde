...
class EventoForm(AcessibilidadeMixin, forms.ModelForm):
    class Meta:
        ...
        labels = {
            ...
            "link_ingressos": "Link externo de ingressos",
        }
        help_texts = {
...
class EventoPainelForm(EventoForm):
    class Meta(EventoForm.Meta):
        fields = EventoForm.Meta.fields + ["status", "destaque", "motivo_rejeicao"]
        widgets = {
            **EventoForm.Meta.widgets,
            "motivo_rejeicao": forms.Textarea(attrs={"rows": 3}),
        }
        labels = {
            **EventoForm.Meta.labels,
            "status": "Status",
...
class LocalForm(AcessibilidadeMixin, forms.ModelForm):
    class Meta:
        model = Local
        fields = [
            "nome", "endereco", "referencia", "regiao", "cidade", "cep",
            "descricao", "site", "latitude", "longitude",
            "acessivel_cadeirante", "estacionamento",
        ]
        widgets = {
        ...
        help_texts = {
            "latitude": "Deixe em branco para geocodificar automaticamente pelo endereço.",
        }
...
class ProdutorPainelForm(ProdutorForm):
    email_do_dono = forms.CharField(
        label="Vincular à conta de e-mail",
        required=False,
        help_text="Quem entrar com este e-mail poderá editar este perfil. Deixe em branco para não vincular a ninguém.",
    )

    class Meta(ProdutorForm.Meta):
        fields = ProdutorForm.Meta.fields + ["verificado"]

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["email_do_dono"].widget.attrs.setdefault("class", "campo")
        if self.instance.pk and self.instance.user_id:
            self.fields["email_do_dono"].initial = self.instance.user.email

    def clean_email_do_dono(self):
        email = self.cleaned_data["email_do_dono"].strip().lower()
        if not email:
            return None
        try:
            return User.objects.get(email__iexact=email)
        except User.DoesNotExist:
            raise forms.ValidationError("Não existe conta com este e-mail.")

    def save(self, commit=True):
        produtor = super().save(commit=False)
        produtor.user = self.cleaned_data.get("email_do_dono")
        if commit:
            produtor.save()
        return produtor
...
class SolicitacaoDestaqueForm(AcessibilidadeMixin, forms.ModelForm):
    plano = forms.ChoiceField(
        label="Plano",
        choices=[(p.value, p.label) for p in PlanoDestaque if p != PlanoDestaque.NORMAL],
        widget=forms.RadioSelect,
    )

    class Meta:
        model = SolicitacaoDestaque
        fields = ["plano", "mensagem"]
        widgets = {
            "mensagem": forms.Textarea(
                attrs={"rows": 3, "placeholder": "Alguma preferência de data ou observação? (opcional)"}
            ),
        }
        labels = {"mensagem": "Mensagem para a equipe"}

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["mensagem"].required = False
        self.fields["mensagem"].widget.attrs.setdefault("class", "campo")
