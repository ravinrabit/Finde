...
class EventoQuerySet(models.QuerySet):
    ...
    def com_rank_destaque(self):
        agora = timezone.now()
        condicoes = [
            models.When(plano_destaque=plano, destaque_pago_ate__gte=agora, then=models.Value(valor))
            for plano, valor in RANK_POR_PLANO.items()
            if valor
        ]
        rank = models.Case(*condicoes, default=models.Value(0), output_field=models.IntegerField())
        return self.annotate(rank_destaque_db=rank)
...
class Evento(models.Model):
    ...
    plano_destaque = models.CharField(
        max_length=20, choices=PlanoDestaque.choices, default=PlanoDestaque.NORMAL,
    )
    destaque_pago_ate = models.DateTimeField(null=True, blank=True)
    ...
    def _montar_busca(self):
        ...
        cidade = self.cidade if self.cidade != "Brasília" else ""
        partes = [
            self.nome, self.resumo, self.descricao, self.local,
            self.endereco, organizador, cidade,
    ...
    @property
    def destaque_ativo(self):
        return (
            self.plano_destaque != PlanoDestaque.NORMAL
            and self.destaque_pago_ate is not None
            and self.destaque_pago_ate >= timezone.now()
        )

    @property
    def rank_destaque(self):
        return RANK_POR_PLANO.get(self.plano_destaque, 0) if self.destaque_ativo else 0
...
class SolicitacaoDestaque(models.Model):
    evento = models.ForeignKey(
        Evento, on_delete=models.CASCADE, related_name="solicitacoes_destaque"
    )
    plano = models.CharField(max_length=20, choices=PlanoDestaque.choices)
    ...
    def __str__(self):
        return f"{self.evento.nome} — {self.get_plano_display()}"
